  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">论文阶段</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">核心诉求</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">学术论文开题与研究方法通关指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 学术论文写作与开题报告专家');
...
const metricsList = [
  {
    "key": "academicProposalRigor",
    "label": "开题报告严谨度"
  },
  {
    "key": "methodologyFeasibility",
    "label": "研究方法可行性"
  },
  {
    "key": "literatureReviewIntegration",
    "label": "文献融合深度"
  },
  {
    "key": "academicStylePolish",
    "label": "学术语体润色分"
  },
  {
    "key": "thesisStructureClarity",
    "label": "论文结构清晰度"
  }
];
const guideRules = [
  {
    "title": "研究方法与问题精准对齐",
    "advice": "如果研究问题是“态度与意向”，优先采用问卷量表与结构方程模型 (SEM)；如果是“机制影响”，使用实证面板数据",
    "avoid": "切忌研究方法与研究问题张冠李戴"
  },
  {
    "title": "创新点要实事求是",
    "advice": "创新点写 2-3 个即可（如“视角创新”、“数据集创新”），用客观中立语言表述",
    "avoid": "避免大言不惭宣称“填补了国内空白”等过头话"
  },
  {
    "title": "学术降重重在句式重构",
    "advice": "采用被动句改主动句、同义替换及调整论述顺序进行降重，保持学术严谨性",
    "avoid": "严禁使用无逻辑的同义词乱替换导致语句不通顺"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[XUESHU_SCORES\][\s\S]*?\[\/XUESHU_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[XUESHU_SCORES\](.*?)\[\/XUESHU_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xueshu_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xueshu_history_records', JSON.stringify(historyList.value));
};
